# monte_carlo_event_sim_real_time_v8_separation.py
# WARNING: Includes NAIVE O(N) separation check - EXPECT EXTREME SLOWDOWN.
#          Requires spatial indexing for practical use.
# Real-time Monte Carlo with Size, Priority-Size, Separation, Pop Density.
...
PLOT_UPDATE_INTERVAL_SEC = 2.0 # Increased due to expected slowness
...
# --- New: Event Size and Separation ---
EVENT_RADIUS_RANGES_METERS = { # ASSUMPTION: Radius ranges per category
    'Low': (20, 25),
    'Medium': (25, 30),
    'High': (30, 35),
    'Very_High': (35, 40)
}
MINIMUM_SEPARATION_METERS = 50.0 # Center-to-center distance
...
PLOT3_CMAP = 'magma_r'
PLOT1_MARKERSIZE = 2 # Slightly larger markers

PLACEMENT_RULE = 'PopulationWeighted'

active_events = [] # Stores: [id, category_idx, x, y, expiry_time, radius]
event_id_counter = 0
...
def create_dummy_density_grid(grid_info):
    ...
    dummy_grid = dummy_grid * 10000 # Scale up
...
class EventGenerator:
    def __init__(self, category_rates, radius_ranges, event_duration_sec, placement_rule,
        ...
        self.min_separation = min_separation # New separation distance
        ...
        self.placement_attempts_per_event = 200 # Increase attempts due to separation constraint
    ...
    def _check_separation(self, x_cand, y_cand, current_active_events):
        if self.min_separation <= 0:
            return True # No separation required
        for _, _, ex, ey, _, _ in current_active_events: # Check existing x, y
             # Faster distance check (squared) - avoid sqrt unless needed
             dist_sq = (x_cand - ex)**2 + (y_cand - ey)**2
             if dist_sq < self.min_separation**2:
                 return False # Too close
        return True # Separation ok

    def _place_event(self, current_active_events):
        # Combines placement logic (Uniform or PopWeighted) with separation check
        ...
        for _ in range(self.placement_attempts_per_event):
            location = placement_method() # Try to get a location based on density/uniformity + polygon
            if location:
                x, y = location
                # NOW check separation against current active events
                if self._check_separation(x, y, current_active_events):
                    return x, y # Found valid location meeting all criteria
            # If placement failed or separation check failed, loop continues
        # print("Warning: Failed to place event satisfying separation after max attempts.")
        return None # Failed to place after all attempts

    def _place_event_uniformly(self):
         # Just generates a candidate point within polygon
         for _ in range(100): # Inner attempts for just finding point in polygon
    ...
    def _place_event_population_weighted(self):
        if self.population_density_grid is None or self.max_density <= 0:
            self.placement_rule = 'Uniform'
            return self._place_event_uniformly()
        for _ in range(100): # Inner attempts for acceptance-rejection
    ...
    def generate(self, dt, current_time, current_active_events):
        ...
        category_list = list(self.category_rates.keys()) # Get category names
        ...
        for cat_idx, num_gen in category_counts_to_gen.items():
            ...
            radius_min, radius_max = self.radius_ranges[cat_name]
            for _ in range(num_gen):
                 # Placement attempt loop now incorporates separation check
                 location = self._place_event(current_active_events + new_events) # Check against existing AND newly added in this step
                 ...
                 if location:
                    ...
                    new_event = [event_id_counter, cat_idx, x, y, expiry, radius]
                    new_events.append(new_event)
                    added_count +=1
                 # else: # Optional: Log failure to place due to separation/density
                     # pass

        # Optional: Report if target rate not met due to constraints
        # if added_count < num_to_generate_total:
        #     print(f"Warning: Time {current_time:.1f}s - Added {added_count}/{num_to_generate_total} events due to placement constraints.")
...
class StatsCollector:
    ...
    def sample(self, current_time, active_events, dt):
        ...
        # Check if current time is at or past the next sample time (within tolerance)
        if current_time >= self.warmup_sec and current_time >= self.next_sample_time - dt / 2:
            ...
            for _, cat_idx, x, y, _, _ in active_events: # Ignore expiry, radius for counting
            ...
            # Advance next sample time robustly
...
class RealTimeVisualizer:
    ...
    def _setup_plots(self):
        ...
        self.legend_handles = [] # Store original handles
        for i, cat in enumerate(self.categories):
            line, = ax1.plot([], [], '.', color=self.category_colors(i / self.num_categories), label=cat, markersize=self.params['PLOT1_MARKERSIZE']*2)
            self.legend_handles.append(line)
        ax1.legend(handles=self.legend_handles, loc='upper right', fontsize='small')
        fig1.tight_layout()
    ...
    def update(self, active_events_list, mean_count_grid, prob_grid, current_sim_time, num_snapshots):
        ...
        for _, cat_idx, x, y, _, _ in active_events_list: # Include radius index 5, although unused in plot
    ...
    def close(self):
        if MATPLOTLIB_AVAILABLE:
            plt.ioff()
            for fig_id in plt.get_fignums(): plt.close(fig_id)
...
def run_simulation_main_loop(polygon, prepared, grid_info, warmup_duration_sec):
    ...
    event_generator = EventGenerator(
        CATEGORY_RATES, EVENT_RADIUS_RANGES_METERS, EVENT_DURATION_SEC, PLACEMENT_RULE,
        density_grid, grid_info, prepared.contains, MINIMUM_SEPARATION_METERS # Pass separation
    )
    ...
    try:
        while True:
            ...
            active_events = [e for e in active_events if current_time < e[4]] # Expire
            newly_generated = event_generator.generate(TIME_STEP_SEC, current_time, active_events) # Generate (passes current active for sep check)
            active_events.extend(newly_generated) # Add new ones
            sampled_this_step = stats_collector.sample(current_time, active_events, TIME_STEP_SEC) # Sample stats
            prob_grid = stats_collector.get_probabilities() # Get latest stats
            mean_grid = stats_collector.get_expected_values()
            visualizer.update(active_events, mean_grid, prob_grid, current_time, stats_collector.total_snapshots) # Update plots
            current_time += TIME_STEP_SEC # Advance time
...
if __name__ == "__main__":
    ...
    print("WARNING: Naive O(N) separation check active - expect very slow performance.")
    try:
        print("="*60)
        print("Real-Time MC Sim w/ Size, Separation, Pop. Density (v8)")
        print("="*60)
        poly, prep, grid = load_area_and_setup_grid(MONTREAL_BOUNDARY_FILE, TARGET_CRS, CELL_SIZE_METERS)
        run_simulation_main_loop(poly, prep, grid, WARMUP_DURATION_SEC)
    except FileNotFoundError as e: print(f"Error: {e}"); print("Please ensure the boundary file exists.")
